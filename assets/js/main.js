...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Dynamically compute relative path to /assets/js/testimonials.json
  const depth = window.location.pathname.split("/").filter(Boolean).length;
  const prefix = "../".repeat(depth);
  const jsonPath = `${prefix}assets/js/testimonials.json`;
  ...
  function fadeAndLoad(testimonials) {
    ...
    buttonContainer.querySelector("a").addEventListener("click", (e) => {
      e.preventDefault();
      container.innerHTML = "";
      fadeAndLoad(testimonials); // Reload different ones
    });
  }
  ...
  fetch(jsonPath)
    .then((res) => res.json())
    .then((data) => {
      if (data.testimonials?.length) fadeAndLoad(data.testimonials);
    })
